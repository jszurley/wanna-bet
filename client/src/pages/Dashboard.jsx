...
import { getUpcomingBets, getPendingBets, getUncompletedBets, getCompletedBets, getRejectedBets, getDisputedBets } from '../services/api';
...
export default function Dashboard() {
  ...
  const [counts, setCounts] = useState({ upcoming: 0, pending: 0, uncompleted: 0, completed: 0, rejected: 0, disputed: 0 });
  ...
  const loadCounts = async () => {
    try {
      const [upcoming, pending, uncompleted, completed, rejected, disputed] = await Promise.all([
        getUpcomingBets(),
        getPendingBets(),
        getUncompletedBets(),
        getCompletedBets(),
    ...
    try {
      ...
      switch (activeTab) {
        ...
        case 'rejected':
          ...
          break;
        case 'disputed':
          response = await getDisputedBets();
          break;
        default:
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <p>
            ...
            {activeTab === 'disputed' && "No disputed bets."}
